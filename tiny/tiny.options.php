		...
		class tinyOptions {
			...
			public function section( $group_id, $group ) {
				...
				foreach ( $group['fields'] as $field_id => $field ) {
					...
					if ( isset( $field['fields'] ) ) {
						$field['args']['fields'] = $field['fields'];
					}
					add_settings_field(
						$field_id,
						$field['title'],
			...
			public function sanitize( $new_values ) {
				...
				foreach ( $this->settings['sections'] as $section_id => $section ) {
					...
					foreach( $section['fields'] as $field_id => $field ) {
						...
						if ( 'group' != $field['callback'] ) {
							continue;
						}
						foreach( $field['fields'] as $f_id => $f ) {
							$new_values[ $field_id ][ $f_id ] = apply_filters(  $this->settings['page']['option']."_sanitize_{$field_id}_{$f_id}", $new_values[ $field_id ][ $f_id ], $f );
							if ( !isset( $f['callback'] ) ) {
								continue;
							}
							if ( 'checkbox' != $f['callback'] ) {
								continue;
							}
							if ( isset( $new_values[ $field_id ][ $f_id ] ) ) {
								continue;
							}
							$new_values[ $field_id ][ $f_id ] = false;
						}
					}
				}
			...
			public function group( $args ) {
				...
				foreach ( $args['fields'] as $field_id => $field ) {
					$field['args']['option_id'] = $field_id;
					$field['args']['parent_id'] = $args['option_id'];
					$field['args']['title'] = $field['title'];
					$field['args']['description'] = isset( $field['description'] ) ? $field['description'] : false;
					$field['args']['label'] = isset( $field['label'] ) ? $field['label'] : false;
					$field['args']['list'] = isset( $field['list'] ) ? $field['list'] : false;
					...
					echo '<p><strong>'.$field['title'].'</strong><br/>';
			...
			public function input( $args ) {
				...
				$tag_args = wp_parse_args(
					$args['attributes'],
					array(
						'id'	=> ( isset( $args['parent_id'] ) ? $args['parent_id'].'_' : '' ) . $args['option_id'],
						'name' => $this->settings['page']['option'].( isset( $args['parent_id'] ) ? ( '['.$args['parent_id'].']') : '' ) ."[{$args['option_id']}]",
						'type'	=> 'text',
						'value'	=> isset( $args['parent_id'] ) ? ( $this->_get_value( $args['parent_id'] )[ $args['option_id' ] ] ) : $this->_get_value( $args['option_id'] ),
						'class'	=> 'regular-text',
					)
				);
				$tag_args = $this->_print_attributes( $tag_args );
